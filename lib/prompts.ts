...
/** Estimates a salary range based on job description and candidate CV. Non-streaming. */
export function buildSalaryEstimatePrompt(
  jobDescription: string,
  texContent: string,
): string {
  const rules = `You are an expert compensation analyst. Analyze the job description and the candidate's CV to estimate a salary range for this role.
